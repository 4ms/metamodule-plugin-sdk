...
DWtype __negdi2(DWtype u) {
  ...
  const DWunion w = {
      {.low = -uu.s.low, .high = -uu.s.high - ((UWtype)-uu.s.low > 0)}};
...
SItype __addvsi3(SItype a, SItype b) {
  SItype w;
...
#ifdef L_addvdi3
DWtype __addvDI3(DWtype a, DWtype b) {
  DWtype w;
...
#ifdef L_subvsi3
Wtype __subvSI3(Wtype a, Wtype b) {
  Wtype w;
...
#ifdef COMPAT_SIMODE_TRAPPING_ARITHMETIC
SItype __subvsi3(SItype a, SItype b) {
  SItype w;
...
#ifdef L_subvdi3
DWtype __subvDI3(DWtype a, DWtype b) {
  DWtype w;
...
#ifdef L_mulvsi3
Wtype __mulvSI3(Wtype a, Wtype b) {
  Wtype w;
...
DWtype __mulvDI3(DWtype u, DWtype v) {
  ...
  if (__builtin_expect(uu.s.high == uu.s.low >> (W_TYPE_SIZE - 1), 1)) {
    ...
    if (__builtin_expect(vv.s.high == vv.s.low >> (W_TYPE_SIZE - 1), 1)) {
      ...
      /* Two multiplications.  */
      DWunion w0 = {.ll =
                        (UDWtype)(UWtype)uu.s.low * (UDWtype)(UWtype)vv.s.low};
      DWunion w1 = {.ll =
                        (UDWtype)(UWtype)uu.s.low * (UDWtype)(UWtype)vv.s.high};
    ...
    if (__builtin_expect(vv.s.high == vv.s.low >> (W_TYPE_SIZE - 1), 1)) {
      ...
      DWunion w0 = {.ll =
                        (UDWtype)(UWtype)uu.s.low * (UDWtype)(UWtype)vv.s.low};
      DWunion w1 = {.ll =
                        (UDWtype)(UWtype)uu.s.high * (UDWtype)(UWtype)vv.s.low};
...
/* Unless shift functions are defined with full ANSI prototypes,
   parameter b will be promoted to int if shift_count_type is smaller than an
   int.  */
#ifdef L_lshrdi3
...
#ifdef L_bswapsi2
SItype __bswapsi2(SItype u) {
  return ((((u) & 0xff000000u) >> 24) | (((u) & 0x00ff0000u) >> 8) |
...
#if (defined(L_udivdi3) || defined(L_divdi3) || defined(L_umoddi3) ||          \
     defined(L_moddi3))
...
UWtype __udiv_w_sdiv(UWtype *rp __attribute__((__unused__)),
...
#if (defined(L_udivdi3) || defined(L_divdi3) || defined(L_umoddi3) ||          \
     defined(L_moddi3) || defined(L_divmoddi4))
#define L_udivmoddi4
#endif
...
#ifdef L_cmpdi2
cmp_return_type __cmpdi2(DWtype a, DWtype b) { return (a > b) - (a < b) + 1; }
#endif

#ifdef L_ucmpdi2
cmp_return_type __ucmpdi2(UDWtype a, UDWtype b) {
  return (a > b) - (a < b) + 1;
}
#endif

#if defined(L_fixunstfdi) && LIBGCC2_HAS_TF_MODE
UDWtype __fixunstfDI(TFtype a) {
  if (a < 0)
    return 0;
...
XFtype __floatundixf(UDWtype u) {
#if W_TYPE_SIZE > __LIBGCC_XF_MANT_DIG__
#error
#endif
...
TFtype __floatunditf(UDWtype u) {
#if W_TYPE_SIZE > __LIBGCC_TF_MANT_DIG__
#error
#endif
...
#define F_MODE_OK(SIZE)                                                        \
  (SIZE < DI_SIZE && SIZE > (DI_SIZE - SIZE + FSSIZE) &&                       \
...
#define REP_BIT ((UDWtype)1 << (DI_SIZE - FSIZE))
  ...
  /* Protect against double-rounding error.
     ...
     configurations.  */
  if (!(-((DWtype)1 << FSIZE) < u && u < ((DWtype)1 << FSIZE))) {
...
void __eprintf(const char *string, const char *expression, unsigned int line,
               const char *filename) {
...
void __clear_cache(void *beg __attribute__((__unused__)),
